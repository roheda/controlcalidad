...
export default function MainNavigation() {
    ...
    <>
      ...
      <div style={{ display: "grid", gap: 10 }}>
        {groups.map((group) => {
          ...
          return <button key={group.id} type="button" onMouseEnter={() => setHovered(group.id)} onClick={() => handleGroup(group)} title={group.label} style={{ width: 56, height: 56, border: active || open ? "2px solid #007aff" : "1px solid rgba(60,60,67,0.14)", borderRadius: 20, background: active || open ? "rgba(0,122,255,0.10)" : "rgba(255,255,255,0.92)", color: active || open ? "#005ecb" : "#1d1d1f", cursor: "pointer", boxShadow: active ? "0 8px 20px rgba(0,122,255,0.18)" : "0 8px 18px rgba(0,0,0,0.06)", display: "grid", placeItems: "center", fontWeight: 950, fontSize: 19 }}>{group.icon}</button>;
        })}
      </div>
    </>
  );
  ...
  const mobileNav = (
    <>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10, marginBottom: 18 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}><img src="/triton-logo.png" alt="Triton" style={{ width: 38, height: 38, objectFit: "contain", borderRadius: 12, background: "#111" }} /><div><div style={{ fontSize: 20, fontWeight: 950 }}>TRITON OS</div><div style={{ color: "#6e6e73", fontSize: 12 }}>Operación integral</div></div></div>
      ...
      <div style={{ display: "grid", gap: 9 }}>
        {groups.map((group) => {
          ...
          return <div key={group.id}>
            <button type="button" onClick={() => group.children?.length ? setMobileGroups((v) => ({ ...v, [group.id]: !v[group.id] })) : goTo(group.module || group.id, group.os)} style={{ width: "100%", border: active ? "2px solid #007aff" : "1px solid rgba(60,60,67,0.12)", borderRadius: 18, padding: 13, background: active ? "rgba(0,122,255,0.10)" : "rgba(255,255,255,0.86)", color: active ? "#005ecb" : "#1d1d1f", cursor: "pointer", display: "grid", gridTemplateColumns: "34px 1fr 18px", gap: 10, alignItems: "center", textAlign: "left" }}>
              <span style={{ width: 34, height: 34, borderRadius: 12, display: "inline-flex", alignItems: "center", justifyContent: "center", background: active ? "#007aff" : "#f2f2f7", color: active ? "#fff" : "#1d1d1f", fontWeight: 950 }}>{group.icon}</span>
              <span><span style={{ display: "block", fontWeight: 950, fontSize: 14 }}>{group.label}</span><span style={{ display: "block", color: "#6e6e73", fontSize: 12, marginTop: 3 }}>{group.helper}</span></span>
              {group.children?.length ? <span style={{ color: "#6e6e73", fontWeight: 950 }}>{opened ? "⌃" : "⌄"}</span> : null}
            </button>
  ...
  return <>
    ...
    {showFlyout ? <div onMouseEnter={() => setHovered(flyoutGroup.id)} onMouseLeave={() => { if (!pinned) setHovered(null); }} style={{ position: "fixed", left: sidebarWidth + 12, top: 70 + groups.findIndex((g) => g.id === flyoutGroup.id) * 66, zIndex: 2147483647, width: 330, maxWidth: "calc(100vw - 120px)", background: "rgba(255,255,255,0.98)", border: "1px solid rgba(60,60,67,0.16)", borderRadius: 24, boxShadow: "0 22px 70px rgba(0,0,0,0.18)", padding: 12, WebkitBackdropFilter: "blur(18px) saturate(180%)", backdropFilter: "blur(18px) saturate(180%)" }}>
      <div style={{ padding: "8px 10px 12px", borderBottom: "1px solid rgba(60,60,67,0.10)", marginBottom: 8 }}><div style={{ fontWeight: 950, fontSize: 16 }}>{flyoutGroup.label}</div><div style={{ color: "#6e6e73", fontSize: 12, marginTop: 3 }}>{flyoutGroup.helper}</div></div>
      <div style={{ display: "grid", gap: 4 }}>{flyoutGroup.children.map((child) => <button key={child.id} type="button" onClick={() => goTo(child.id, child.os)} style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: 8, textAlign: "left", border: activeModule === child.id ? "1px solid #007aff" : "1px solid transparent", borderRadius: 16, padding: "11px 12px", background: activeModule === child.id ? "rgba(0,122,255,0.09)" : "transparent", cursor: "pointer", alignItems: "center" }}><span><span style={{ display: "block", fontWeight: 950, color: activeModule === child.id ? "#005ecb" : "#1d1d1f", fontSize: 14 }}>{child.label}</span><span style={{ display: "block", color: "#6e6e73", fontSize: 12, marginTop: 2 }}>{child.helper}</span></span><span style={{ color: "#8e8e93", fontWeight: 950 }}>›</span></button>)}</div>
    </div> : null}
