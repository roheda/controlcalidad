...
function RentalContractForm({ data, tenantMap, assetMap, form, setForm, onSave, editing }) {
  ...
  return <div style={{ marginTop: 12, display: "grid", gap: 12 }}>
    ...
    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(190px,1fr))", gap: 10 }}>
      ...
      <Field label="Renta base"><input type="number" style={inputStyle()} value={form.rentBase || ""} onChange={(e) => setForm({ ...form, rentBase: e.target.value })} /></Field>
      <Field label="Mantenimiento %"><input type="number" style={inputStyle()} value={form.maintenancePct ?? 0} onChange={(e) => setForm({ ...form, maintenancePct: e.target.value })} /></Field>
...
function Permits({ data, projectMap, rows, addRecord, updateRecord, showForm, setShowForm, form, setForm }) {
  ...
  function preloadProjectPermits() {
    if (!missingTemplates.length) { alert("Este proyecto ya tiene cargados los trámites del catálogo."); return; }
    if (!window.confirm(`Se crearán ${missingTemplates.length} trámite(s) para ${projectMap[projectId]?.name || projectId}.`)) return;
  ...
  return <div style={{ display: "grid", gap: 16 }}>
    <Card><div style={{ display: "flex", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}><SectionTitle title="Trámites por proyecto" helper="Los trámites se precargan desde Catálogos y reglas. Aquí solo se da seguimiento, se registran avances y se conserva historial." /><Button variant="secondary" onClick={preloadProjectPermits}>Precargar trámites del catálogo</Button></div>
