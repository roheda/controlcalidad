...
function currentFinanceUser(data) {
  const email = firebaseAuth.currentUser?.email?.toLowerCase?.() || "";
  const record = (data?.users || []).find((u) => String(u.email || "").toLowerCase() === email);
  if (record) return { email, role: record.active === false ? "usuario" : (record.role || "usuario"), name: record.name || email };
  // Respaldo de arranque: solo aplica mientras nadie ha dado de alta usuarios reales en "Usuarios".
...
function canFinanceAction(action, data) {
  const { role } = currentFinanceUser(data);
...
function PaymentContextModal({ row, data, projectMap, categoryMap, onClose, onAuthorize, onCorrection, onReject }) {
  ...
  const canAuthorizeFinal = canFinanceAction("authorize", data);
...
function Payables({ data, projectMap, categoryMap, rows, addRecord, updateRecord, deleteRecord, showForm, setShowForm, form, setForm }) {
  ...
  const currentUser = currentFinanceUser(data);
  ...
  const canAdminOperate = canFinanceAction("adminReview", data);
...
function Authorizations({ data, projectMap, categoryMap, updateRecord }) {
  ...
  const canAuthorize = canFinanceAction("authorize", data);
  ...
  function authorizeRow(r) { updateRecord("payables", r.id, { status: "Autorizado", authorizedBy: currentFinanceUser(data).email || "rodrigo@tritondesarrollos.com", authorizedAt: todayIso() }); }
...
function PettyCash({ data, projectMap, categoryMap, addRecord, updateRecord, showForm, setShowForm, form, setForm }) {
  ...
  function sendReplenishment(lines) {
    ...
    addRecord("payables", { projectId: replenishCash.projectId, supplierId: "", supplier: `Reposición caja chica - ${replenishCash.responsible}`, concept: `Reposición de caja chica · ${replenishCash.name}`, categoryId: "caja_chica", contractId: "", paymentStage: "Reposición caja chica", amount: roundMoney(totalBase), iva: roundMoney(totalIva), retention: roundMoney(totalRetention), requestedBy: currentFinanceUser(data).email || replenishCash.responsible, requestedByName: currentFinanceUser(data).name || replenishCash.responsible, requiredDate: todayIso(), status: "Solicitado", priority: "Media", documentStatus: attachments.length ? "Soporte cargado" : "Pendiente anexos", attachments, adminReviewed: false, notes: `Reposición ligada a caja ${replenishCash.name}`, pettyCashId: replenishCash.id, pettyCashResponsible: replenishCash.responsible, createdAt: todayIso() });
...
function permissionTemplate(role = "usuario", modules = {}) {
  ...
  if (role === "constructora") return matrixWith(modules, ["dashboard", "obras_calidad", "estimaciones"], ["view", "create", "edit"]);
...
function roleLabel(role) {
  const labels = { master: "Master", finanzas_pagos: "Finanzas / pagos", supervisora: "Supervisión", constructora: "Constructora", cobranza: "Cobranza", gestoria: "Gestoría", usuario: "Usuario consulta" };
