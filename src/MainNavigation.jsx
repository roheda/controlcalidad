...
const modules = [
  ...
  { id: "obras", label: "Obras", helper: "Configuración de obra", icon: "⌂" },
];
...
function openFeedbackModule(moduleId) {
  ...
  const menuButton = Array.from(document.querySelectorAll("button[aria-label='Abrir menú']")).find(
    (button) => !button.classList.contains("triton-shell-menu-button")
  );

  if (menuButton) {
    menuButton.click();
    window.setTimeout(() => clickButtonByText(label), 80);
  }
}

function closeEstimacionesIfOpen() {
...
export default function MainNavigation() {
  ...
  const styleTag = useMemo(
    () => `
      ...
      button[aria-label='Abrir menú']:not(.triton-shell-menu-button) {
  ...
  function goTo(moduleId) {
    ...
    closeEstimacionesIfOpen();
    window.setTimeout(() => openFeedbackModule(moduleId), 80);
  }
  ...
  const navContent = (
    <>
      ...
      <div style={{ display: "grid", gap: 2 }}>
        {modules.map((module) => (
          <NavButton key={module.id} module={module} active={activeModule === module.id} collapsed={collapsed && isDesktop} onClick={() => goTo(module.id)} />
        ))}
      </div>
    </>
  );
  ...
  return (
    <>
      ...
      <aside
        className="triton-shell-sidebar"
        style={{
          position: "fixed",
          left: 0,
          top: 0,
          ...
          width: sidebarWidth,
          zIndex: 2147483646,
          padding: collapsed ? "18px 10px" : "22px 14px",
      ...
      <button
        className="triton-shell-menu-button"
        type="button"
        ...
        >
          ...
          >
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
              <div>
                <div style={{ fontSize: 22, fontWeight: 950 }}>Triton OS</div>
                <div style={{ color: "#6e6e73", fontSize: 13 }}>Módulos</div>
              </div>
              <button type="button" onClick={() => setMobileOpen(false)} style={{ border: "1px solid rgba(60,60,67,0.12)", background: "#fff", borderRadius: 999, width: 36, height: 36, fontWeight: 950 }}>
                ×
              </button>
            </div>
            <div style={{ display: "grid", gap: 4 }}>
              {modules.map((module) => (
                <NavButton key={module.id} module={module} active={activeModule === module.id} collapsed={false} onClick={() => goTo(module.id)} />
              ))}
            </div>
          </div>
        </div>
