...
const approvedLotStatuses = ["lista_administracion", "administracion_revision", "pago_programado", "pagada", "parcial_aprobada"];
...
function groupByPartida(items) { return items.reduce((acc, item) => { const key = item.partida || "General"; if (!acc[key]) acc[key] = []; acc[key].push(item); return acc; }, {}); }
function todayDelay(fechaEntrega) { if (!fechaEntrega) return 0; const today = new Date(); const entrega = new Date(fechaEntrega); return today > entrega ? Math.ceil((today - entrega) / (1000 * 60 * 60 * 24)) : 0; }
...
function badge(status) { const colors = { borrador: ["#eef2f7", "#475467"], en_aprobacion: ["#fff3cd", "#9a6700"], parcial_aprobada: ["#fff3cd", "#9a6700"], lista_administracion: ["#e8f7ed", "#157347"], administracion_revision: ["#eef2ff", "#3730a3"], pago_programado: ["#e8f7ed", "#157347"], pagada: ["#e8f7ed", "#157347"], rechazada: ["#fdecec", "#b42318"] }; const [bg, color] = colors[status] || colors.borrador; return { display: "inline-flex", borderRadius: 999, padding: "6px 10px", background: bg, color, fontSize: 12, fontWeight: 900 }; }
...
export default function EstimacionesWidget() {
  ...
  const [lotForm, setLotForm] = useState({ numero: 1, nombre: "", periodo: new Date().toISOString().slice(0, 7) });
  ...
  const nextNumber = lots.map((lot) => Number(lot.numero || 0)).filter(Boolean).length ? Math.max(...lots.map((lot) => Number(lot.numero || 0)).filter(Boolean)) + 1 : 1;
  const visibleLot = selectedLot || approvalLots[0] || lots[0] || null;

  useEffect(() => { const handler = () => { setOpen(true); setActiveTab("captura"); }; window.addEventListener("triton-open-estimaciones", handler); window.addEventListener("triton-module-estimaciones", handler); return () => { window.removeEventListener("triton-open-estimaciones", handler); window.removeEventListener("triton-module-estimaciones", handler); }; }, []);
  ...
  function updateDraft(conceptId, patch) { const concept = catalog.find((item) => item.id === conceptId); const available = concept ? availableFor(concept) : 100; setDraftRows((prev) => ({ ...prev, [conceptId]: { ...(prev[conceptId] || {}), ...patch, percent: patch.percent !== undefined ? Math.min(available, clampPercent(patch.percent)) : (prev[conceptId]?.percent || "") } })); }
  function buildRowsFromDraft() {
    return catalog.map((concept) => {
      ...
      return { rowId: `${concept.id}-${Date.now()}`, conceptId: concept.id, clave: concept.clave, partida: concept.partida, concepto: concept.concepto, unidad: concept.unidad, cantidad: Number(concept.cantidad || 0), precioUnitario: Number(concept.precioUnitario || 0), importeConcepto: Number(concept.importe || 0), avancePrevioAprobado: previo, avanceDisponibleAntes: Math.max(0, 100 - previo), avanceSolicitado: percent, avanceAprobado: 0, importeSolicitado: Number(concept.importe || 0) * (percent / 100), importeAprobado: 0, comentarioConstructora: draftRows[concept.id]?.comment || "", fechaEntrega: concept.fechaEntrega || "", diasAtraso: todayDelay(concept.fechaEntrega), multa, status: "borrador" };
    }).filter(Boolean);
  }
  function computeRowsTotals(rows, onlyApproved = false) { const baseRows = onlyApproved ? rows.filter((row) => row.status === "aprobada_supervision") : rows.filter((row) => row.status !== "rechazada_supervision"); const subtotal = baseRows.reduce((acc, row) => acc + Number(row.importeAprobado || row.importeSolicitado || 0), 0); return { subtotal, ...computeDeductions(subtotal, baseRows) }; }
  function houseStatusFromRows(rows) { if (!rows.length) return "borrador"; const approved = rows.filter((row) => row.status === "aprobada_supervision").length; const rejected = rows.filter((row) => row.status === "rechazada_supervision").length; if (approved === rows.length) return "aprobada_supervision"; if (approved > 0 || rejected > 0) return "parcial_supervision"; return "en_aprobacion"; }
  function housePayload(house, rows, houseStatus = "borrador") { const totals = computeRowsTotals(rows, false); return { id: house.id, houseId: house.id, houseName: house.name || house.id, block: house.block || "", status: houseStatus, rows, totals }; }
  function lotTotals(housesObject) { return Object.values(housesObject || {}).reduce((acc, house) => ({ subtotal: acc.subtotal + Number(house.totals?.subtotal || 0), amortizacion: acc.amortizacion + Number(house.totals?.amortizacion || 0), retencion: acc.retencion + Number(house.totals?.retencion || 0), multas: acc.multas + Number(house.totals?.multas || 0), neto: acc.neto + Number(house.totals?.neto || 0), houses: acc.houses + 1 }), { subtotal: 0, amortizacion: 0, retencion: 0, multas: 0, neto: 0, houses: 0 }); }
  function lotStatusFromHouses(housesObject) { const allRows = Object.values(housesObject || {}).flatMap((house) => house.rows || []); if (!allRows.length) return "en_aprobacion"; const approved = allRows.filter((row) => row.status === "aprobada_supervision").length; const rejected = allRows.filter((row) => row.status === "rechazada_supervision").length; if (approved === allRows.length) return "lista_administracion"; if (approved > 0 || rejected > 0) return "parcial_aprobada"; return "en_aprobacion"; }

  async function saveDraftLot() { if (!selectedHouse) { alert("Selecciona una casa base."); return; } const rows = buildRowsFromDraft(); if (!rows.length) { alert("Captura al menos un porcentaje de avance."); return; } const id = currentLotId || `estimacion-${lotForm.numero || nextNumber}-${Date.now()}`; const houseData = housePayload(selectedHouse, rows, "borrador"); const housesObject = { [selectedHouse.id]: houseData }; const lot = { id, obraId: selectedObraId, obraName: selectedObra.name || selectedObraId, numero: Number(lotForm.numero || nextNumber), nombre: lotForm.nombre || `Estimación ${lotForm.numero || nextNumber} · ${selectedObra.name || selectedObraId}`, periodo: lotForm.periodo, status: "borrador", estimationConfig: { anticipoPorcentaje, retencionPorcentaje, multaDiaria }, houses: housesObject, totals: lotTotals(housesObject), createdAt: serverTimestamp(), updatedAt: serverTimestamp() }; await setDoc(doc(db, "obras", selectedObraId, "estimacionLotes", id), lot, { merge: true }); setCurrentLotId(id); alert("Borrador guardado como lote formal."); await loadData(); setActiveTab("borradores"); }
  async function copyLotToHouses(lot, targetIds = copyHouseIds) { if (!lot || !targetIds.length) { alert("Selecciona casas destino."); return; } const sourceHouse = Object.values(lot.houses || {})[0]; if (!sourceHouse) return; const housesObject = { ...(lot.houses || {}) }; targetIds.forEach((houseId) => { const house = houses.find((item) => item.id === houseId); if (!house) return; const copiedRows = (sourceHouse.rows || []).map((row) => ({ ...row, rowId: `${row.conceptId}-${houseId}-${Date.now()}`, status: "borrador", avanceAprobado: 0, importeAprobado: 0 })); housesObject[houseId] = housePayload(house, copiedRows, "borrador"); }); await setDoc(doc(db, "obras", selectedObraId, "estimacionLotes", lot.id), { houses: housesObject, totals: lotTotals(housesObject), updatedAt: serverTimestamp() }, { merge: true }); setCopyHouseIds([]); await loadData(); }
  async function sendLotToApproval(lot) { const housesObject = {}; Object.entries(lot.houses || {}).forEach(([houseId, house]) => { const rows = (house.rows || []).map((row) => ({ ...row, status: "en_aprobacion" })); housesObject[houseId] = { ...house, rows, status: "en_aprobacion", totals: computeRowsTotals(rows, false) }; }); await setDoc(doc(db, "obras", selectedObraId, "estimacionLotes", lot.id), { houses: housesObject, status: "en_aprobacion", totals: lotTotals(housesObject), submittedAt: serverTimestamp(), updatedAt: serverTimestamp() }, { merge: true }); await loadData(); }
  function loadLotToCapture(lot) { const firstHouse = Object.values(lot.houses || {})[0]; if (!firstHouse) return; setSelectedHouseId(firstHouse.houseId); const nextDraft = {}; (firstHouse.rows || []).forEach((row) => { nextDraft[row.conceptId] = { percent: row.avanceSolicitado, comment: row.comentarioConstructora || "" }; }); setDraftRows(nextDraft); setCurrentLotId(lot.id); setLotForm({ numero: lot.numero, nombre: lot.nombre, periodo: lot.periodo }); setActiveTab("captura"); }
  async function reviewRows(lot, houseId, rowIds, approved, comment = "") { if (!lot || !houseId || !rowIds.length) return; const housesObject = { ...(lot.houses || {}) }; const house = housesObject[houseId]; const rows = (house.rows || []).map((row) => { const id = row.rowId || row.conceptId; if (!rowIds.includes(id)) return row; return { ...row, status: approved ? "aprobada_supervision" : "rechazada_supervision", avanceAprobado: approved ? Number(row.avanceSolicitado || 0) : 0, importeAprobado: approved ? Number(row.importeSolicitado || 0) : 0, comentarioSupervision: comment, reviewedAt: new Date().toISOString() }; }); const nextHouseStatus = houseStatusFromRows(rows); housesObject[houseId] = { ...house, rows, status: nextHouseStatus, comentarioSupervision: comment, reviewedAt: new Date().toISOString(), totals: computeRowsTotals(rows, nextHouseStatus !== "en_aprobacion") }; const nextStatus = lotStatusFromHouses(housesObject); await setDoc(doc(db, "obras", selectedObraId, "estimacionLotes", lot.id), { houses: housesObject, status: nextStatus, totals: lotTotals(housesObject), reviewedAt: serverTimestamp(), updatedAt: serverTimestamp() }, { merge: true }); setSelectedReviewRowIds((prev) => ({ ...prev, [houseId]: [] })); await loadData(); }
  async function reviewHouse(lot, houseId, approved) { const house = lot.houses?.[houseId]; const rowIds = (house?.rows || []).filter((row) => row.status === "en_aprobacion").map((row) => row.rowId || row.conceptId); await reviewRows(lot, houseId, rowIds, approved); }
  async function setAdminStatus(lot, status) { await setDoc(doc(db, "obras", selectedObraId, "estimacionLotes", lot.id), { status, adminUpdatedAt: serverTimestamp(), updatedAt: serverTimestamp() }, { merge: true }); await loadData(); }
  ...
  return <div className="triton-estimaciones-module" style={{ position: "fixed", left: "var(--triton-shell-offset, 84px)", top: 0, right: 0, bottom: 0, zIndex: 2147483644, background: "#f5f5f7", overflow: "auto" }}>
    ...
    <div style={{ maxWidth: 1480, margin: "0 auto", padding: "calc(24px + env(safe-area-inset-top, 0px)) 18px 42px" }}>
      <div style={{ display: "flex", justifyContent: "space-between", gap: 16, alignItems: "flex-start", marginBottom: 18, flexWrap: "wrap" }}>
        <div><div style={{ fontSize: 34, fontWeight: 950, color: "#1d1d1f", letterSpacing: -0.7 }}>Estimaciones</div><div style={{ color: "#6e6e73", fontSize: 16, marginTop: 6 }}>Borrador por partidas, catálogo configurado en Obras, amortización proporcional y seguimiento hasta pago.</div></div>
        <button type="button" onClick={() => setOpen(false)} style={{ ...buttonBase, background: "#fff", color: "#1d1d1f" }}>Volver a Calidad</button>
      </div>

      <Card title="Contexto del lote" subtitle="El catálogo y el anticipo se configuran en Obras. Aquí solo se captura la estimación contra el catálogo vigente.">
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(185px, 1fr))", gap: 12 }}>
          ...
          <Field label="Nombre del lote"><input value={lotForm.nombre} onChange={(e) => setLotForm({ ...lotForm, nombre: e.target.value })} placeholder={`Estimación ${lotForm.numero} ${selectedObra.name || selectedObraId}`} style={inputBase} /></Field>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(170px, 1fr))", gap: 12, marginBottom: 16 }}>
        ...
        <Metric label="Aprobado acumulado" value={money(approvedTotal)} helper="Ya no disponible" />
      ...
      {activeTab === "captura" ? <Card title="Captura por partidas" subtitle="La captura respeta el acumulado aprobado. La amortización del anticipo se descuenta desde el borrador.">
      ...
      {activeTab === "borradores" ? <Card title="Borradores de estimación" subtitle="Abre el borrador, cópialo a otras casas, guarda copias y envía el lote a aprobación.">{draftLots.length ? draftLots.map((lot) => <div key={lot.id} style={{ padding: 14, borderRadius: 18, border: "1px solid rgba(60,60,67,0.12)", background: "#fff", marginBottom: 12 }}><div style={{ display: "flex", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}><div><div style={{ fontWeight: 950 }}>{lot.nombre || `Estimación ${lot.numero}`}</div><div style={{ color: "#6e6e73", fontSize: 12, marginTop: 4 }}>{lot.periodo} · {Object.keys(lot.houses || {}).length} casa(s) · Bruto {money(lot.totals?.subtotal)} · Amortización {money(lot.totals?.amortizacion)} · Neto {money(lot.totals?.neto)}</div></div><span style={badge(lot.status)}>{statusLabel[lot.status]}</span></div><div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 12 }}><button type="button" onClick={() => loadLotToCapture(lot)} style={{ ...buttonBase, background: "#fff" }}>Abrir / editar</button><button type="button" onClick={() => copyLotToHouses(lot)} disabled={!copyHouseIds.length} style={{ ...buttonBase, background: copyHouseIds.length ? "#007aff" : "#e5e5ea", color: copyHouseIds.length ? "#fff" : "#8e8e93" }}>Copiar a casas seleccionadas</button><button type="button" onClick={() => sendLotToApproval(lot)} style={{ ...buttonBase, background: "#34c759", color: "#fff" }}>Enviar lote a aprobación</button></div><div style={{ marginTop: 12, display: "flex", gap: 8, flexWrap: "wrap" }}>{houses.filter((house) => !lot.houses?.[house.id]).map((house) => <button key={house.id} type="button" onClick={() => toggleCopyHouse(house.id)} style={{ ...buttonBase, background: copyHouseIds.includes(house.id) ? "#007aff" : "#fff", color: copyHouseIds.includes(house.id) ? "#fff" : "#1d1d1f" }}>{house.name || house.id}</button>)}</div></div>) : <div style={{ color: "#6e6e73" }}>No hay borradores guardados.</div>}</Card> : null}

      {activeTab === "aprobacion" ? <Card title="Aprobación ingeniería" subtitle="El supervisor puede aprobar conceptos parciales dentro de cada casa, no necesariamente toda la casa.">{visibleLot && approvalLots.length ? <>{Object.values(visibleLot.houses || {}).map((house) => { const rowsByPartida = groupByPartida(house.rows || []); const selectedRows = selectedReviewRowIds[house.houseId] || []; return <div key={house.houseId} style={{ border: "1px solid rgba(60,60,67,0.12)", borderRadius: 18, background: "#fff", marginBottom: 12, overflow: "hidden" }}><div style={{ display: "flex", justifyContent: "space-between", gap: 12, alignItems: "center", padding: 14, background: "rgba(242,242,247,0.8)", flexWrap: "wrap" }}><div><div style={{ fontWeight: 950 }}>{house.houseName}</div><div style={{ color: "#6e6e73", fontSize: 12 }}>Subtotal aprobado: {money(house.totals?.subtotal)} · Estado {house.status}</div></div><div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}><button type="button" onClick={() => selectAllRowsForHouse(house)} style={{ ...buttonBase, background: "#fff" }}>Seleccionar pendientes</button><button type="button" onClick={() => reviewRows(visibleLot, house.houseId, selectedRows, true)} disabled={!selectedRows.length} style={{ ...buttonBase, background: selectedRows.length ? "#34c759" : "#e5e5ea", color: selectedRows.length ? "#fff" : "#8e8e93" }}>Aprobar selección</button><button type="button" onClick={() => reviewRows(visibleLot, house.houseId, selectedRows, false)} disabled={!selectedRows.length} style={{ ...buttonBase, background: selectedRows.length ? "#ff3b30" : "#e5e5ea", color: selectedRows.length ? "#fff" : "#8e8e93" }}>Rechazar selección</button><button type="button" onClick={() => reviewHouse(visibleLot, house.houseId, true)} style={{ ...buttonBase, background: "#e8f7ed", color: "#157347" }}>Aprobar casa completa</button></div></div>{Object.entries(rowsByPartida).map(([partida, rows]) => <div key={`${house.houseId}-${partida}`} style={{ padding: 12, borderTop: "1px solid rgba(60,60,67,0.10)" }}><div style={{ fontWeight: 950, marginBottom: 8 }}>{partida}</div><div style={{ overflowX: "auto" }}><table style={{ width: "100%", minWidth: 980, borderCollapse: "collapse" }}><thead><tr><th style={{ ...th, textAlign: "center" }}>Sel.</th><th style={{ ...th, textAlign: "left" }}>Clave</th><th style={{ ...th, textAlign: "left" }}>Concepto</th><th style={{ ...th, textAlign: "right" }}>Previo</th><th style={{ ...th, textAlign: "right" }}>Solicitado</th><th style={{ ...th, textAlign: "right" }}>Importe</th><th style={{ ...th, textAlign: "left" }}>Comentario</th><th style={{ ...th, textAlign: "left" }}>Estatus</th></tr></thead><tbody>{rows.map((row) => { const rowId = row.rowId || row.conceptId; const disabled = row.status !== "en_aprobacion"; return <tr key={rowId}><td style={{ ...td, textAlign: "center" }}><input type="checkbox" disabled={disabled} checked={selectedRows.includes(rowId)} onChange={() => toggleReviewRow(house.houseId, rowId)} /></td><td style={{ ...td, fontWeight: 900 }}>{row.clave}</td><td style={{ ...td, minWidth: 300 }}>{row.concepto}</td><td style={{ ...td, textAlign: "right" }}>{Number(row.avancePrevioAprobado || 0).toFixed(1)}%</td><td style={{ ...td, textAlign: "right", fontWeight: 900 }}>{Number(row.avanceSolicitado || 0).toFixed(1)}%</td><td style={{ ...td, textAlign: "right", fontWeight: 950 }}>{money(row.importeSolicitado)}</td><td style={td}>{row.comentarioConstructora || "-"}</td><td style={td}><span style={badge(row.status || "en_aprobacion")}>{row.status || "en_aprobacion"}</span></td></tr>; })}</tbody></table></div></div>)}</div>; })}</> : <div style={{ color: "#6e6e73" }}>No hay lotes enviados a aprobación.</div>}</Card> : null}

      {activeTab === "estatus" ? <Card title="Estatus de estimaciones" subtitle="La estimación viaja como lote formal. Administración y constructora ven el mismo lote, monto y estado.">{adminLots.length || lots.length ? (adminLots.length ? adminLots : lots).map((lot) => <div key={lot.id} style={{ padding: 14, borderRadius: 18, border: "1px solid rgba(60,60,67,0.12)", background: "#fff", marginBottom: 12 }}><div style={{ display: "flex", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}><div><div style={{ fontWeight: 950 }}>{lot.nombre || `Estimación ${lot.numero} · ${selectedObraId}`}</div><div style={{ color: "#6e6e73", fontSize: 12, marginTop: 4 }}>{lot.periodo} · {Object.keys(lot.houses || {}).length} casa(s) · Bruto {money(lot.totals?.subtotal)} · Amortización {money(lot.totals?.amortizacion)} · Neto {money(lot.totals?.neto)}</div></div><span style={badge(lot.status)}>{statusLabel[lot.status] || lot.status}</span></div>{Object.values(lot.houses || {}).map((house) => <details key={house.houseId} style={{ marginTop: 10 }}><summary style={{ cursor: "pointer", fontWeight: 900 }}>{house.houseName} · {house.status} · {money(house.totals?.neto)}</summary><div style={{ marginTop: 8, overflowX: "auto" }}><table style={{ width: "100%", minWidth: 860, borderCollapse: "collapse" }}><thead><tr><th style={{ ...th, textAlign: "left" }}>Clave</th><th style={{ ...th, textAlign: "left" }}>Concepto</th><th style={{ ...th, textAlign: "right" }}>Solicitado</th><th style={{ ...th, textAlign: "right" }}>Aprobado</th><th style={{ ...th, textAlign: "right" }}>Importe aprobado</th><th style={{ ...th, textAlign: "left" }}>Estatus</th></tr></thead><tbody>{(house.rows || []).map((row) => <tr key={row.rowId || row.conceptId}><td style={{ ...td, fontWeight: 900 }}>{row.clave}</td><td style={td}>{row.concepto}</td><td style={{ ...td, textAlign: "right" }}>{Number(row.avanceSolicitado || 0).toFixed(1)}%</td><td style={{ ...td, textAlign: "right" }}>{Number(row.avanceAprobado || 0).toFixed(1)}%</td><td style={{ ...td, textAlign: "right" }}>{money(row.importeAprobado)}</td><td style={td}>{row.status}</td></tr>)}</tbody></table></div></details>)}<div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 12 }}><button type="button" onClick={() => setAdminStatus(lot, "administracion_revision")} style={{ ...buttonBase, background: "#fff" }}>En revisión admin</button><button type="button" onClick={() => setAdminStatus(lot, "pago_programado")} style={{ ...buttonBase, background: "#007aff", color: "#fff" }}>Pago programado</button><button type="button" onClick={() => setAdminStatus(lot, "pagada")} style={{ ...buttonBase, background: "#34c759", color: "#fff" }}>Pagada</button></div></div>) : <div style={{ color: "#6e6e73" }}>No hay estimaciones todavía.</div>}</Card> : null}
    </div>
  </div>;
}
