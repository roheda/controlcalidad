...
function rowsToCatalog(rows, sourceFileName = "") {
  ...
  return rows.slice(1).map((row, index) => {
    ...
    return normalizeCatalogItem({
      ...
      fechaEntrega: raw.fechaEntrega || raw["Fecha compromiso"] || raw["fecha compromiso"] || raw.fecha_entrega,
      rowNumber: index + 2,
      sourceFileName,
    }, index);
...
export default function EstimacionesWidget() {
  ...
  function availableFor(concept, houseId = selectedHouseId) {
    return Math.max(0, 100 - approvedFor(houseId, concept.id));
  }
  ...
  }, [catalog, draftRows, lotForm, selectedHouseId, approvedProgress]);
  ...
  const approvedTotal = useMemo(() => {
    return houses.reduce((houseAcc, house) => houseAcc + catalog.reduce((acc, concept) => acc + approvedAmountFor(concept, house.id), 0), 0);
  }, [houses, catalog, approvedProgress]);
  ...
  function computeRowsTotals(rows, onlyApproved = false) {
    const baseRows = onlyApproved ? rows.filter((row) => row.status === "aprobada_supervision") : rows.filter((row) => row.status !== "rechazada_supervision");
    const subtotal = baseRows.reduce((acc, row) => acc + Number(row.importeAprobado || row.importeSolicitado || 0), 0);
    const multas = baseRows.reduce((acc, row) => acc + Number(row.multa || 0), 0);
    const amortizacion = subtotal * (parseNumber(lotForm.anticipoPorcentaje) / 100);
    const retencion = subtotal * (parseNumber(lotForm.retencionPorcentaje) / 100);
    const neto = Math.max(0, subtotal - amortizacion - retencion - multas);
    return { subtotal, amortizacion, retencion, multas, neto };
  }
  function houseStatusFromRows(rows) {
    if (!rows.length) return "borrador";
    const approved = rows.filter((row) => row.status === "aprobada_supervision").length;
    const rejected = rows.filter((row) => row.status === "rechazada_supervision").length;
    if (approved === rows.length) return "aprobada_supervision";
    if (approved > 0 && rejected > 0) return "parcial_supervision";
    if (approved > 0) return "parcial_supervision";
    if (rejected === rows.length) return "rechazada_supervision";
    return "en_aprobacion";
  }
  function housePayload(house, rows, houseStatus = "borrador") {
    const totals = computeRowsTotals(rows, false);
    return { id: house.id, houseId: house.id, houseName: house.name || house.id, block: house.block || "", status: houseStatus, rows, totals };
  }
  function lotTotals(housesObject) {
    const list = Object.values(housesObject || {});
    return list.reduce((acc, house) => ({
  ...
  function lotStatusFromHouses(housesObject) {
    const housesList = Object.values(housesObject || {});
    const allRows = housesList.flatMap((house) => house.rows || []);
    if (!allRows.length) return "en_aprobacion";
    const approved = allRows.filter((row) => row.status === "aprobada_supervision").length;
    const pending = allRows.filter((row) => row.status === "en_aprobacion").length;
    if (approved === allRows.length) return "lista_administracion";
    if (approved > 0 || pending > 0) return "parcial_aprobada";
    return "rechazada";
  }
  ...
  async function saveDraftLot() {
    ...
    await setDoc(doc(db, "obras", selectedObraId, "estimacionLotes", id), {
      id,
      obraId: selectedObraId,
      numero: Number(lotForm.numero || 1),
      nombre: name,
      periodo: lotForm.periodo,
      status: "borrador",
      form: lotForm,
      baseHouseId: selectedHouse.id,
      houses: housesObject,
      totals,
    ...
    alert("Borrador guardado. Puedes verlo en Borradores para copiarlo a otras casas o enviarlo a aprobación.");
    await loadData();
  }
  function loadLotToCapture(lot, houseId) {
    ...
    setLotForm({ ...lot.form, numero: lot.numero, nombre: lot.nombre, periodo: lot.periodo });
  ...
  async function copyLotToHouses(lot, targetIds) {
    ...
    targets.forEach((house) => {
      const rows = (sourceHouse.rows || []).map((row) => {
        const approved = approvedFor(house.id, row.conceptId);
        const available = Math.max(0, 100 - approved);
        const nextPercent = Math.min(available, Number(row.avanceSolicitado || 0));
        return {
          ...row,
          rowId: `${row.conceptId}-${house.id}-${Date.now()}`,
          avancePrevioAprobado: approved,
          avanceDisponibleAntes: available,
          avanceSolicitado: nextPercent,
          importeSolicitado: Number(row.importeConcepto || 0) * (nextPercent / 100),
          avanceAprobado: 0,
          importeAprobado: 0,
          status: "borrador",
        };
      }).filter((row) => row.avanceSolicitado > 0);
      housesObject[house.id] = housePayload(house, rows, "borrador");
    });
    ...
    setCopyHouseIds([]);
    alert(`Borrador copiado a ${targets.length} casa(s).`);
    await loadData();
  }
  async function sendLotToApproval(lot) {
    const housesObject = {};
    Object.values(lot.houses || {}).forEach((house) => {
      housesObject[house.houseId] = { ...house, status: "en_aprobacion", rows: (house.rows || []).map((row) => ({ ...row, status: "en_aprobacion" })) };
    });
    ...
    alert("Estimación enviada a aprobación de supervisión.");
    await loadData();
  ...
  async function reviewRows(lot, houseId, rowIds, approved) {
    ...
    const comment = window.prompt(approved ? "Comentario de aprobación parcial" : "Motivo de rechazo parcial", "") || "";
    const housesObject = { ...(lot.houses || {}) };
    const house = housesObject[houseId];
    if (!house) return;
    const rows = (house.rows || []).map((row) => {
      if (!rowIds.includes(row.rowId || row.conceptId)) return row;
      return {
        ...row,
        status: approved ? "aprobada_supervision" : "rechazada_supervision",
    ...
    const nextHouseStatus = houseStatusFromRows(rows);
    housesObject[houseId] = { ...house, rows, status: nextHouseStatus, comentarioSupervision: comment, reviewedAt: new Date().toISOString(), totals: computeRowsTotals(rows, nextHouseStatus !== "en_aprobacion") };
    const nextStatus = lotStatusFromHouses(housesObject);
  ...
  function toggleReviewRow(houseId, rowId) {
    setSelectedReviewRowIds((prev) => {
      const current = prev[houseId] || [];
      return { ...prev, [houseId]: current.includes(rowId) ? current.filter((item) => item !== rowId) : [...current, rowId] };
    });
  }
  function selectAllRowsForHouse(house) {
    setSelectedReviewRowIds((prev) => ({ ...prev, [house.houseId]: (house.rows || []).filter((row) => row.status === "en_aprobacion").map((row) => row.rowId || row.conceptId) }));
  }
  ...
  return <div className="triton-estimaciones-module" style={{ position: "fixed", left: "var(--triton-shell-offset, 84px)", top: 0, right: 0, bottom: 0, zIndex: 2147483644, background: "#f5f5f7", overflow: "auto" }}>
    ...
    <div style={{ maxWidth: 1480, margin: "0 auto", padding: "calc(24px + env(safe-area-inset-top, 0px)) 18px 42px" }}>
      <div style={{ display: "flex", justifyContent: "space-between", gap: 16, alignItems: "flex-start", marginBottom: 18, flexWrap: "wrap" }}>
        <div><div style={{ fontSize: 34, fontWeight: 950, color: "#1d1d1f", letterSpacing: -0.7 }}>Estimaciones</div><div style={{ color: "#6e6e73", fontSize: 16, marginTop: 6 }}>Borrador por partidas, acumulado aprobado por concepto, aprobación parcial y seguimiento hasta pago.</div></div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(170px, 1fr))", gap: 12, marginBottom: 16 }}>
        ...
        <Metric label="Borrador actual" value={money(draftSummary.subtotal)} helper={`${draftSummary.count} conceptos`} />
        <Metric label="Amortización" value={money(draftSummary.amortizacion)} helper={`${lotForm.anticipoPorcentaje}%`} />
        <Metric label="Retención" value={money(draftSummary.retencion)} helper={`${lotForm.retencionPorcentaje}%`} />
        <Metric label="Neto borrador" value={money(draftSummary.neto)} />
      </div>

      {activeTab === "captura" ? <Card title="Captura por partidas" subtitle="La captura respeta el acumulado aprobado. Solo podrás estimar el porcentaje disponible de cada concepto por casa.">
        {Object.entries(catalogByPartida).map(([partida, concepts]) => {
          ...
          return <div key={partida} style={{ border: "1px solid rgba(60,60,67,0.13)", borderRadius: 20, overflow: "hidden", background: "#fff", marginBottom: 14 }}>
            ...
            {!collapsed ? <div style={{ overflowX: "auto" }}><table style={{ width: "100%", minWidth: 1260, borderCollapse: "collapse" }}><thead><tr><th style={{ ...th, textAlign: "left" }}>Clave</th><th style={{ ...th, textAlign: "left" }}>Concepto</th><th style={{ ...th, textAlign: "left" }}>Unidad</th><th style={{ ...th, textAlign: "right" }}>Unidades</th><th style={{ ...th, textAlign: "right" }}>P.U.</th><th style={{ ...th, textAlign: "right" }}>Total</th><th style={{ ...th, textAlign: "right" }}>Aprobado</th><th style={{ ...th, textAlign: "right" }}>Disponible</th><th style={{ ...th, textAlign: "center" }}>% estimar</th><th style={{ ...th, textAlign: "right" }}>A estimar</th><th style={{ ...th, textAlign: "left" }}>Fecha compromiso</th><th style={{ ...th, textAlign: "left" }}>Comentario</th></tr></thead><tbody>{concepts.map((concept) => {
              const approved = approvedFor(selectedHouseId, concept.id);
              const available = availableFor(concept);
              return <tr key={concept.id}><td style={{ ...td, fontWeight: 900 }}>{concept.clave}</td><td style={{ ...td, minWidth: 300 }}>{concept.concepto}</td><td style={td}>{concept.unidad}</td><td style={{ ...td, textAlign: "right" }}>{Number(concept.cantidad || 0).toLocaleString("es-MX")}</td><td style={{ ...td, textAlign: "right" }}>{money(concept.precioUnitario)}</td><td style={{ ...td, textAlign: "right", fontWeight: 850 }}>{money(concept.importe)}</td><td style={{ ...td, textAlign: "right", color: approved > 0 ? "#157347" : "#6e6e73", fontWeight: 850 }}>{approved.toFixed(1)}%</td><td style={{ ...td, textAlign: "right", color: available <= 0 ? "#b42318" : "#005ecb", fontWeight: 850 }}>{available.toFixed(1)}%</td><td style={{ ...td, textAlign: "center" }}><input type="number" min="0" max={available} value={draftRows[concept.id]?.percent || ""} disabled={available <= 0} onChange={(e) => updateDraft(concept.id, { percent: e.target.value })} style={{ ...inputBase, width: 88, textAlign: "center", opacity: available <= 0 ? 0.55 : 1 }} /></td><td style={{ ...td, textAlign: "right", fontWeight: 950, color: draftPercent(concept) > 0 ? "#007aff" : "#6e6e73" }}>{money(plannedAmount(concept))}</td><td style={td}>{concept.fechaEntrega || "Opcional"}</td><td style={td}><input value={draftRows[concept.id]?.comment || ""} onChange={(e) => updateDraft(concept.id, { comment: e.target.value })} placeholder="Soporte" style={{ ...inputBase, minHeight: 38, padding: "8px 10px" }} /></td></tr>;
            })}</tbody></table><div style={{ padding: 12, background: "rgba(0,122,255,0.05)", borderTop: "1px solid rgba(60,60,67,0.10)", fontWeight: 900 }}>Subtotal partida: {money(partidaPlaneada)} / {money(partidaTotal)}</div></div> : null}
          </div>;
        })}
      ...
      {activeTab === "aprobacion" ? <Card title="Aprobación de ingeniería" subtitle="Revisión parcial: puedes aprobar o rechazar conceptos/partidas dentro de cada casa, no necesariamente la casa completa.">{approvalLots.length ? <><div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 12 }}>{approvalLots.map((lot) => <button key={lot.id} type="button" onClick={() => { setSelectedLotId(lot.id); setSelectedReviewRowIds({}); }} style={{ ...buttonBase, background: visibleLot?.id === lot.id ? "#007aff" : "#fff", color: visibleLot?.id === lot.id ? "#fff" : "#1d1d1f" }}>{lot.nombre || `Estimación ${lot.numero}`}</button>)}</div>{visibleLot ? <><div style={{ display: "flex", justifyContent: "space-between", gap: 12, flexWrap: "wrap", marginBottom: 12 }}><div><div style={{ fontWeight: 950, fontSize: 18 }}>{visibleLot.nombre}</div><div style={{ color: "#6e6e73", fontSize: 12 }}>{visibleLot.periodo} · Neto aprobado/parcial {money(visibleLot.totals?.neto)}</div></div></div>{Object.values(visibleLot.houses || {}).map((house) => {
        ...
        })}</> : null}</> : <div style={{ color: "#6e6e73" }}>No hay lotes enviados a aprobación.</div>}</Card> : null}

      {activeTab === "estatus" ? <Card title="Estatus de estimaciones" subtitle="La estimación viaja como lote formal. Administración y constructora ven el mismo lote, monto y estado.">{adminLots.length || lots.length ? (adminLots.length ? adminLots : lots).map((lot) => <div key={lot.id} style={{ padding: 14, borderRadius: 18, border: "1px solid rgba(60,60,67,0.12)", background: "#fff", marginBottom: 12 }}><div style={{ display: "flex", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}><div><div style={{ fontWeight: 950 }}>{lot.nombre || `Estimación ${lot.numero} · ${selectedObraId}`}</div><div style={{ color: "#6e6e73", fontSize: 12, marginTop: 4 }}>{lot.periodo} · {Object.keys(lot.houses || {}).length} casa(s) · Neto {money(lot.totals?.neto)}</div></div><span style={badge(lot.status)}>{statusLabel[lot.status] || lot.status}</span></div>{Object.values(lot.houses || {}).map((house) => <details key={house.houseId} style={{ marginTop: 10 }}><summary style={{ cursor: "pointer", fontWeight: 900 }}>{house.houseName} · {house.status} · {money(house.totals?.neto)}</summary><div style={{ marginTop: 8, overflowX: "auto" }}><table style={{ width: "100%", minWidth: 860, borderCollapse: "collapse" }}><thead><tr><th style={{ ...th, textAlign: "left" }}>Clave</th><th style={{ ...th, textAlign: "left" }}>Concepto</th><th style={{ ...th, textAlign: "right" }}>Solicitado</th><th style={{ ...th, textAlign: "right" }}>Aprobado</th><th style={{ ...th, textAlign: "right" }}>Importe aprobado</th><th style={{ ...th, textAlign: "left" }}>Estatus</th></tr></thead><tbody>{(house.rows || []).map((row) => <tr key={row.rowId || row.conceptId}><td style={{ ...td, fontWeight: 900 }}>{row.clave}</td><td style={td}>{row.concepto}</td><td style={{ ...td, textAlign: "right" }}>{Number(row.avanceSolicitado || 0).toFixed(1)}%</td><td style={{ ...td, textAlign: "right" }}>{Number(row.avanceAprobado || 0).toFixed(1)}%</td><td style={{ ...td, textAlign: "right" }}>{money(row.importeAprobado)}</td><td style={td}>{row.status}</td></tr>)}</tbody></table></div></details>)}<div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 12 }}><button type="button" onClick={() => setAdminStatus(lot, "administracion_revision")} style={{ ...buttonBase, background: "#fff" }}>En revisión admin</button><button type="button" onClick={() => setAdminStatus(lot, "pago_programado")} style={{ ...buttonBase, background: "#007aff", color: "#fff" }}>Pago programado</button><button type="button" onClick={() => setAdminStatus(lot, "pagada")} style={{ ...buttonBase, background: "#34c759", color: "#fff" }}>Pagada</button></div></div>) : <div style={{ color: "#6e6e73" }}>No hay estimaciones todavía.</div>}</Card> : null}

      {activeTab === "catalogo" ? <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(320px, 1fr))", gap: 16 }}><Card title="Importar catálogo CSV" subtitle="Formato esperado: PARTIDA, clave, descripcion, Unidades, unidad, P.U. También puede traer Fecha compromiso opcional."><Field label="Archivo CSV"><input type="file" accept=".csv,text/csv" onChange={(e) => importCatalogFile(e.target.files?.[0])} /></Field><button type="button" onClick={seedCatalog} style={{ ...buttonBase, background: "#fff" }}>Cargar catálogo demo</button>{importingCatalog ? <div style={{ color: "#6e6e73", marginTop: 10 }}>Importando catálogo...</div> : null}{catalogImportInfo ? <div style={{ marginTop: 12, padding: 12, borderRadius: 16, background: "#e8f7ed", color: "#157347", fontWeight: 850 }}>{catalogImportInfo.rows} conceptos importados · {catalogImportInfo.partidas} partidas · {money(catalogImportInfo.total)}</div> : null}</Card><Card title="Alta manual de concepto" subtitle="Útil para extras o conceptos no incluidos en catálogo."><Field label="Clave"><input value={manualConcept.clave} onChange={(e) => setManualConcept({ ...manualConcept, clave: e.target.value })} style={inputBase} /></Field><Field label="Partida"><input value={manualConcept.partida} onChange={(e) => setManualConcept({ ...manualConcept, partida: e.target.value })} style={inputBase} /></Field><Field label="Concepto"><textarea value={manualConcept.concepto} onChange={(e) => setManualConcept({ ...manualConcept, concepto: e.target.value })} rows={3} style={{ ...inputBase, resize: "vertical" }} /></Field><div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}><Field label="Unidad"><input value={manualConcept.unidad} onChange={(e) => setManualConcept({ ...manualConcept, unidad: e.target.value })} style={inputBase} /></Field><Field label="Unidades"><input type="number" value={manualConcept.cantidad} onChange={(e) => setManualConcept({ ...manualConcept, cantidad: e.target.value })} style={inputBase} /></Field></div><Field label="Precio unitario"><input type="number" value={manualConcept.precioUnitario} onChange={(e) => setManualConcept({ ...manualConcept, precioUnitario: e.target.value })} style={inputBase} /></Field><Field label="Fecha compromiso opcional"><input type="date" value={manualConcept.fechaEntrega} onChange={(e) => setManualConcept({ ...manualConcept, fechaEntrega: e.target.value })} style={inputBase} /></Field><button type="button" onClick={addManualConcept} style={{ ...buttonBase, background: "#007aff", color: "#fff" }}>Agregar concepto</button></Card></div> : null}
    </div>
  </div>;
}
