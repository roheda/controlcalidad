...
export default function EstimacionesWidget() {
  ...
  function renderAprobacion() {
    ...
    const toggleHouseSelection = (houseId, house) => {
      const ids = reviewableIds(house);
      const selected = selectedReviewRowIds[houseId] || [];
      const allSelected = ids.length > 0 && ids.every((id) => selected.includes(id));
      setSelectedReviewRowIds((prev) => ({
        ...prev,
        [houseId]: allSelected ? [] : ids,
      }));
    };
            ...
            <Card key={houseId} title={house.houseName || houseId} subtitle={`${pendingIds.length} pendiente(s) · ${approvedCount} aprobada(s) · ${observedCount} observada(s)`}>
              ...
              <div style={{ overflowX: "visible" }}>
                <table style={{ width: "100%", borderCollapse: "collapse", tableLayout: "fixed" }}>
                  ...
                  <thead>
                    <tr>
                      <th style={approvalTh}>Sel.</th>
                      <th style={approvalTh}>Estatus</th>
                    ...
                    {(house.rows || []).map((row) => {
                      ...
                      return (
                        <tr key={id} style={{ background: selected ? "#f8fbff" : "#fff" }}>
                          ...
                          <td style={approvalTd}><span style={statusStyle(row.status)}>{rowStatusLabel[row.status] || row.status}</span></td>
