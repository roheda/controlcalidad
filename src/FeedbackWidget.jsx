...
export default function FeedbackWidget() {
  ...
  const [selectedReportObraId, setSelectedReportObraId] = useState(defaultObraId);
  ...
  const selectedReportObra = obras.find((obra) => obra.id === selectedReportObraId) || null;

  useEffect(() => { if (!activeModule) return; loadPanelData(); }, [activeModule, selectedReportObraId]);
  ...
  async function loadPanelData() {
    ...
    try {
      ...
      if (!nextObras.some((obra) => obra.id === selectedReportObraId) && nextObras.length) setSelectedReportObraId(nextObras[0].id);

      const reportObraId = selectedReportObraId || defaultObraId;
      const housesSnap = await getDocs(query(collection(db, "obras", reportObraId, "casas"), orderBy("number", "asc")));
      const nextHouses = await Promise.all(housesSnap.docs.map(async (houseDoc) => {
        const partidasSnap = await getDocs(query(collection(db, "obras", reportObraId, "casas", houseDoc.id, "partidas"), orderBy("weight", "asc")));
  ...
  return (
    <>
      ...
      {activeModule === "reportes" ? <ModulePage title="Reportes" subtitle="Dashboard ejecutivo de avance, revisión, evidencias y problemáticas por obra." onBack={() => setActiveModule("")}>{loadingPanelData ? <div style={{ color: "#6e6e73", marginBottom: 14 }}>Actualizando información...</div> : null}<Card title="Seleccionar obra" subtitle="Las métricas del dashboard se actualizan con base en la obra seleccionada."><div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 10, alignItems: "end" }}><Field label="Obra"><select value={selectedReportObraId} onChange={(event) => setSelectedReportObraId(event.target.value)} style={inputBase}>{obras.length ? obras.map((obra) => <option key={obra.id} value={obra.id}>{obra.name || obra.id} · {obra.status || "sin estatus"}</option>) : <option value={defaultObraId}>Arenna</option>}</select></Field><div style={{ padding: 12, borderRadius: 16, background: "#fff", border: "1px solid rgba(60,60,67,0.12)", marginBottom: 14 }}><div style={{ fontWeight: 950, color: "#1d1d1f" }}>{selectedReportObra?.name || selectedReportObraId}</div><div style={{ color: "#6e6e73", fontSize: 12, marginTop: 4 }}>{selectedReportObra?.location || "Sin ubicación"} · {selectedReportObra?.totalUnits || houses.length || 0} unidades</div></div></div></Card><div style={{ height: 16 }} /><div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 14, marginBottom: 18 }}><Metric label="Avance general" value={`${reports.avgProgress}%`} helper="Promedio por vivienda" /><Metric label="Partidas aprobadas" value={reports.approved} /><Metric label="En revisión" value={reports.review} /><Metric label="Rechazadas" value={reports.rejected} /><Metric label="Riesgo evidencia" value={reports.evidencePending} helper="Partidas con fotos faltantes" /></div><Card title="Problemáticas activas" subtitle="Partidas rechazadas o esperando revisión que requieren seguimiento.">{reports.riskPartidas.length ? reports.riskPartidas.map((partida) => <div key={`${partida.houseName}-${partida.id}`} style={{ display: "flex", justifyContent: "space-between", gap: 12, padding: 14, border: "1px solid rgba(60,60,67,0.12)", borderRadius: 16, background: "#fff", marginBottom: 10 }}><div><div style={{ fontWeight: 900 }}>{partida.houseName} · {partida.name}</div><div style={{ color: "#6e6e73", fontSize: 12, marginTop: 3 }}>Fotos {partida.evidenceCount?.photos || 0} · Videos {partida.evidenceCount?.videos || 0}</div></div><div style={{ color: partida.status === "Rechazada" ? "#ff3b30" : "#9a6700", fontWeight: 900, fontSize: 12 }}>{partida.status}</div></div>) : <div style={{ color: "#6e6e73", fontSize: 14 }}>No hay partidas críticas detectadas en esta obra.</div>}</Card></ModulePage> : null}
